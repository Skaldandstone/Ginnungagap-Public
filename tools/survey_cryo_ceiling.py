...
for actor in unreal.EditorActorSubsystem().get_all_level_actors():
    ...
    # Ceiling band now, not the floor: the clipped pixels turned out to be up there.
    if not (200.0 <= loc.z - DECK_Z <= 460.0):
        continue
...
unreal.log("CRYOCEIL {} distinct floor mesh/material combinations".format(len(seen)))
for (name, mats), entry in sorted(seen.items(), key=lambda kv: -kv[1]["count"]):
    unreal.log("CRYOCEIL x{:<3} {:<22} mats={}  tags={}".format(
        entry["count"], name, list(mats), sorted(entry["tags"])))
unreal.log("CRYOCEIL (* marks a per-actor material override)")
unreal.log("CRYOCEIL done")
