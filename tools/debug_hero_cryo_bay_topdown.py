...
OUTPUT = Path(unreal.SystemLibrary.get_project_saved_directory()) / "HeroCryoBay" / "_debug"
...
def build_shots():
    """Straight down from above, for one diagnostic look at where things actually are."""
    return [(
        "TopDown",
        unreal.Vector(-120.0, -50.0, 900.0),
        unreal.Vector(-120.0, -50.0, 0.0),
        90.0,
    )]
...
def main():
    ...
    def schedule_next():
        ...
        for command in ("ShowFlag.Grid 0", "ShowFlag.Sprites 0", "ShowFlag.Lighting 0",
