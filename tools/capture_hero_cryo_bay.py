...
OUTPUT = Path(unreal.SystemLibrary.get_project_saved_directory()) / "HeroCryoBay"
...
def build_shots():
    """One shot down the real pod row, placed from measured room and pod transforms rather than
    guessed geometry.

    Pods sit at world X=156.2, Y in {-384.3,-136.6,112.2,359.9} -- copied verbatim from
    AProceduralShipBuilder::AddAuthoredCryoRoom. The shell's local bounds (1107.8 x 668.6 x 413.0,
    centred on its own pivot in X/Y) rotate 90 degrees with the room, giving a world footprint of
    X in [-334,334], Y in [-554,554], Z in [-324,89] once placed at the room's (0,0,-300) origin.
    Camera and target are both comfortably inside that box, which the last room's several wrong
    placements were not.
    """
    return [(
        "HeroCryoBay",
        unreal.Vector(0.0, -500.0, -135.0),
        unreal.Vector(156.2, 150.0, -160.0),
        78.0,
    )]
...
def main():
    ...
    for actor in actors_api.get_all_level_actors():
        ...
        if False:  # ceilings kept: they carry this room's only fixtures
            pass
