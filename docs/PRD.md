...
**Canonical playable ship:** `/Game/Assets/Maps/ShipProduction/L_Corvette_ThrustStack` (the GGP-M01
corvette thrust stack, generated by `tools/build_corvette_thrust_stack.py`) - this is what
`UMenuManagerSubsystem::GameModeLevelMap` actually loads for Single Player, Co-op, and Versus from
the Start Game flow, and what `tools/play_corvette.ps1` launches directly. The earlier four-deck
derelict `L_QuickDemo_FourDeck` still exists and is still tested, but is no longer the ship a
player reaches from the menu. `L_Small_Companionway_Showcase` (the fitted four-room pass described in
