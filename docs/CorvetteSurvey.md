...
Written by `Ginnungagap.Survey.CorvetteWalkthrough` on 2026-09-05 16:18 from `/Game/Assets/Maps/ShipProduction/L_Corvette_ThrustStack`. The character was driven on foot up the objective chain, back down it, and out to every side station; everything below was met on the way. Regenerate with the test, do not edit by hand.
...
| 4 | breach patch | 7 | (450, 1707, 2621) | no path | 27 | 0 | arrived, played |
| 5 | CIC access panel | 8 | (830, 910, 3051) | 30.1 | 5 | 0 | arrived, played |
| 6 | CIC console | 8 | (750, 1520, 3051) | 29.3 | 2 | 0 | arrived, played |
...
- Blocked: no complete path from (583, 1395, 3108) to back to the breach patch; nearest obstacle CVT_CorridorDebris (Fallen cable tray, 13.1 m from the path's end). A player cuts, squeezes or overrides here.
- Blocked: no complete path from (1238, 1472, 2248) to side: CVT_Door_CVT-D07; nearest obstacle door CVT_Door_CVT-D07 (2.6 m from the path's end). A player cuts, squeezes or overrides here.
...
- Snag: stuck at (960, 853, 2678) (deck 7) on the way to side: CVT_Door_CVT-D07, 1.6 m short, against nothing solid (a navmesh or path-following stall).
...
- 42 activity stations are static props with a text prompt: each wants a purpose-built asset with an animation for its activity (panel opening, lever, weld arc, console boot).
